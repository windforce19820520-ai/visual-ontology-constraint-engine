...
import type {
  ...
  JsonObject, JsonValue, LocalScenarioPackSource, OutputContract, ScenarioPack,
  ScenarioPackManifest, VersionPin,
} from '@voce/contracts'
import {
  MOCK_JPEG_PLUS_REMOVAL_PROFILE, MOCK_LIMITED_REFERENCE_PROFILE,
  MOCK_NATIVE_TRANSPARENT_PROFILE, compileEvaluationReport, computeExecutionRunHash,
  computeSemanticReviewRequestHash, computeStaticTraceReportModelHash, createHumanAcceptanceDecision,
  createScenarioPackRegistry, executeOffline, executeSemanticReview,
  FixtureSemanticReviewer, guardPromptCandidate, renderStaticTraceReport, sha256,
  traceModelFromExecution, validateStructuralImage, canonicalize,
} from '@voce/core'
import type { OfflineExecutionResult } from '@voce/core'
import type {
  ComparisonSnapshot, EvaluationReport, OfflineExecutionInput,
  PromptCandidateIR, SemanticReviewRequest, StaticTraceReportModel,
} from '@voce/contracts'
import {
  FIXTURE_M6_ALPHA_PNG, fixtureM5Candidate, fixtureM5ExecutionInput, fixtureM5GuardInput, fixtureM5PromptIR, fixtureM6Artifact, fixtureM6Authorization,
} from '@voce/testkit'
import { BUNDLE_MANIFEST_SCHEMA_VERSION } from '@voce/contracts'

export const CLI_VERSION = '0.1.0-rc.1'
const TOOL_ID = '@voce/cli'
const CONTRACTS_VERSION = '0.1.0'
...
function assertNoUnsafe(value: unknown, location = 'document'): void {
  ...
  if (isObject(value)) {
    for (const [key, item] of Object.entries(value)) {
      if (/(?:password|secret|token|api[_-]?key|credential|signed[_-]?url|base64)/i.test(key)) throw new CliError('PUBLIC_SECRET_FIELD_FORBIDDEN', `Sensitive field ${key} is not allowed.`, EXIT.contract)
...
interface SourceFixtureCase { id: string; profileId?: string; expectedStatus?: string; semanticReview?: boolean; humanPending?: boolean; [key: string]: unknown }
...
function makeManifest(doc: SourceDocument, contributions: ScenarioPack['contributions'], files: Array<{ path: string; bytes: Uint8Array }>, suiteList: FixtureSuite[]): ScenarioPackManifest {
  ...
  return {
    ...
    declarations: { containsExecutableScenarioCode: false, distributionLifecycleScripts: false, containsExecutableFiles: false, fixturesRequireNetwork: false, fixturesRequireRealProvider: false, collectsTelemetry: false, mayHandlePersonImages: doc.packId.includes('tryon') || doc.packId.includes('cosplay'), rightsDisclosureRequired: doc.packId.includes('tryon') || doc.packId.includes('cosplay') },
...
function sourceDocument(raw: unknown): SourceDocument {
  ...
  const known = new Set(['schemaVersion', 'packId', 'version', 'kind', 'scenarioLabel', 'contribution', 'contributions', 'fixtures'])
...
async function loadPack(sourcePath: string): Promise<LoadedPack> {
  ...
  const info = await stat(explicit).catch(() => undefined)
  ...
  if (info.isDirectory()) {
    ...
    if (await stat(simple).catch(() => undefined)) inputPath = simple
    else if (await stat(standard).catch(() => undefined)) return loadStandardDirectory(explicit, standard)
    else throw new CliError('PACK_SOURCE_INVALID', 'Explicit directory has no pack.json or scenario-pack/manifest.json.')
  }
  const doc = sourceDocument(await readJson(inputPath))
...
async function loadStandardDirectory(root: string, manifestPath: string): Promise<LoadedPack> {
  const manifest = await readJson(manifestPath) as ScenarioPackManifest
  ...
  for (const item of manifest.distributionInventory) {
    ...
    const filePath = path.join(root, ...item.path.split('/')); const bytes = await readFile(filePath).catch(() => { throw new CliError('PACK_FILE_MISSING', 'ScenarioPack inventory file is missing.', EXIT.contract) })
...
function profileFor(id: string): typeof MOCK_NATIVE_TRANSPARENT_PROFILE {
  if (id === MOCK_NATIVE_TRANSPARENT_PROFILE.id) return MOCK_NATIVE_TRANSPARENT_PROFILE
  if (id === MOCK_JPEG_PLUS_REMOVAL_PROFILE.id) return MOCK_JPEG_PLUS_REMOVAL_PROFILE
...
async function loadProfile(filePath: string): Promise<typeof MOCK_NATIVE_TRANSPARENT_PROFILE> { const object = record(await readJson(path.resolve(filePath)), 'PROFILE_JSON_INVALID'); return profileFor(stringField(object, 'id', 'PROFILE_INVALID')) }
function profilePin(profile: typeof MOCK_NATIVE_TRANSPARENT_PROFILE): VersionPin { return { id: profile.id, version: profile.version, digest: profile.profileHash! } }
...
function defaultCase(doc: Record<string, unknown>, packId: string): CaseSpec {
  ...
  const output: OutputContract = isObject(doc.requestedOutput) ? doc.requestedOutput as unknown as OutputContract : { artifactKind: 'image', dataType: 'image', mediaTypes: ['image/png'], cardinality: { min: 1, max: 1 }, background: 'transparent', allowAlpha: true }
...
interface CompileResult { loaded: LoadedPack; resolution: ReturnType<ReturnType<typeof createScenarioPackRegistry>['resolve']>; caseSpec: CaseSpec; profile: typeof MOCK_NATIVE_TRANSPARENT_PROFILE; fixture: SourceFixtureCase; input: OfflineExecutionInput; prompt: PromptCandidateIR }
async function compileInputs(casePath: string, scenarioPath: string, profilePath: string): Promise<CompileResult> {
  const caseDoc = record(await readJson(path.resolve(casePath)), 'CASE_JSON_INVALID'); const loaded = await loadPack(scenarioPath); const registry = createScenarioPackRegistry(); registry.register(loaded.source)
  ...
  const fixture = loaded.fixtures.find((candidate) => candidate.id === fixtureId) ?? loaded.fixtures[0] ?? { id: fixtureId, profileId: MOCK_NATIVE_TRANSPARENT_PROFILE.id }
  const profile = await loadProfile(profilePath)
  const prompt = fixtureM5Candidate(fixtureM5PromptIR(profile))
  const input = fixtureM5ExecutionInput(profile)
  return { loaded, resolution, caseSpec, profile, fixture, input, prompt }
}

function bundlePins(profile: typeof MOCK_NATIVE_TRANSPARENT_PROFILE, scenario: LoadedPack): BundleManifest['pins'] { return { tool: toolPin(TOOL_ID, CLI_VERSION), core: toolPin('@voce/core', CORE_VERSION), contracts: toolPin('@voce/contracts', CONTRACTS_VERSION), scenario: { id: scenario.descriptor.manifest.packId, version: scenario.descriptor.manifest.version, digest: scenario.descriptor.packageDigest }, profile: profilePin(profile) } }
...
async function validateBundlePath(bundlePath: string): Promise<{ manifest: BundleManifest; payload: Record<string, unknown> }> {
  const directory = path.resolve(bundlePath); const info = await stat(directory).catch(() => undefined); if (!info?.isDirectory()) throw new CliError('BUNDLE_NOT_FOUND', 'Bundle directory was not found.', EXIT.contract)
  const manifest = record(await readJson(path.join(directory, 'manifest.json')), 'BUNDLE_MANIFEST_INVALID') as unknown as BundleManifest
  ...
  for (const entry of files) { const bytes = await readFile(path.join(directory, ...entry.path.split('/'))).catch(() => { throw new CliError('BUNDLE_FILE_MISSING', 'Bundle inventory file is missing.', EXIT.contract) }); if (bytes.byteLength !== entry.byteLength || bytesHash(bytes) !== entry.sha256) throw new CliError('BUNDLE_FILE_HASH_MISMATCH', 'Bundle file hash or length does not match its manifest.', EXIT.contract) }
  ...
  const payload: Record<string, unknown> = {}; for (const entry of files) { if (!entry.path.endsWith('.json')) continue; payload[entry.path.slice(0, -5)] = await readJson(path.join(directory, ...entry.path.split('/'))) } assertNoUnsafe(payload, 'bundle'); return { manifest, payload }
}
...
async function compileCommand(args: Record<string, string>): Promise<JsonObject> {
  const result = await compileInputs(required(args, 'case'), required(args, 'scenario'), required(args, 'profile')); const guardInput = fixtureM5GuardInput(fixtureM5PromptIR(result.profile), result.prompt); const guard = guardPromptCandidate(guardInput)
  const payload = { 'case': result.caseSpec, 'scenario': result.resolution, 'profile': { id: result.profile.id, version: result.profile.version, digest: result.profile.profileHash }, 'fixture': result.fixture, 'constraint-ir': result.input.constraintIR, 'reference-plan': result.input.referencePlan, 'pipeline-plan': result.input.pipelinePlan, 'prompt-ir': result.input.promptArtifact, 'prompt-guard': guard, 'execution-input': result.input, 'replay-contract': { planReplay: 'available', artifactReplay: 'deferred-until-run', liveRerun: 'requires-new-authorization' } }
  const written = await writeBundle(required(args, 'out'), 'compiled', { id: result.caseSpec.id, revision: result.caseSpec.revision }, bundlePins(result.profile, result.loaded), payload)
  return outputSummary(written.manifest, { promptGuard: guard.status, scenario: result.loaded.descriptor.manifest.packId })
}
async function semanticForRun(run: NonNullable<OfflineExecutionResult['run']>, artifacts: ArtifactHandle[], profile: typeof MOCK_NATIVE_TRANSPARENT_PROFILE): Promise<{ report: import('@voce/contracts').SemanticReviewReport; receipt: import('@voce/contracts').StepReceipt; remote: import('@voce/contracts').RemoteCallRun }> {
...
async function runCommand(args: Record<string, string>): Promise<JsonObject> {
  ...
  const bundle = await validateBundlePath(required(args, 'bundle')); if (bundle.manifest.kind !== 'compiled') throw new CliError('BUNDLE_KIND_INVALID', 'case run requires a compiled bundle.', EXIT.contract)
  ...
  const artifacts = run.outputArtifacts ?? []; const structuralArtifact = fixtureM6Artifact(`structural-${run.id}`, FIXTURE_M6_ALPHA_PNG, 'image/png', 'transparent-image'); const outputContract: OutputContract = { artifactKind: 'image', dataType: 'image', mediaTypes: ['image/png'], cardinality: { min: 1, max: 1 }, background: 'transparent', allowAlpha: true }; const structural = validateStructuralImage({ schemaVersion: 'voce.structural-validation-input/v1alpha1', id: `structural-${run.id}`, artifacts: [{ artifact: structuralArtifact, bytes: FIXTURE_M6_ALPHA_PNG }], outputContract, expectedCardinality: { min: 1, max: 1 } });
  const fixture = bundle.payload.fixture as SourceFixtureCase | undefined; let semanticProposal: import('@voce/contracts').SemanticReviewReport | undefined; const semanticReceipts: import('@voce/contracts').StepReceipt[] = []; const reconciliation: import('@voce/contracts').RemoteCallRun[] = []; if (fixture?.semanticReview) { const semantic = await semanticForRun(run, [structuralArtifact, ...artifacts], profileFor(String((bundle.payload.profile as Record<string, unknown>)?.id ?? MOCK_NATIVE_TRANSPARENT_PROFILE.id))); semanticProposal = semantic.report; semanticReceipts.push(semantic.receipt); reconciliation.push(semantic.remote) }
  ...
  const payload = { 'compiled-ref': { semanticHash: bundle.manifest.semanticHash }, 'run': run, 'events': result.events, 'receipts': [...result.receipts, ...semanticReceipts], 'remote-call-runs': [...result.remoteCallRuns, ...reconciliation], 'cleanup': result.cleanupReceipts, 'compensation': result.compensationReceipts, 'evaluation': evaluation, 'trace-model': traceModel, 'comparison-snapshot': { constraintIR: input.constraintIR, referencePlan: input.referencePlan, promptIR: input.promptArtifact, pipelinePlan: input.pipelinePlan, receipts: [...result.receipts, ...semanticReceipts], evaluation } }
  const written = await writeBundle(required(args, 'out'), 'run', bundle.manifest.case, bundle.manifest.pins, payload); return outputSummary(written.manifest, { execution: result.status, executionCode: result.code, evaluation: evaluation.status, traceModel: traceModel.modelHash })
}
async function traceCommand(args: Record<string, string>): Promise<JsonObject> { const bundle = await validateBundlePath(required(args, 'bundle')); const model = bundle.payload['trace-model'] as unknown as StaticTraceReportModel; if (!model || model.schemaVersion !== 'voce.static-trace-report-model/v1alpha1') throw new CliError('TRACE_MODEL_INVALID', 'Bundle has no valid static trace model.', EXIT.contract); const report = renderStaticTraceReport(model); await atomicWrite(path.resolve(required(args, 'out')), report.content); return { status: 'ok', mediaType: report.mediaType, contentHash: report.contentHash, modelHash: report.modelHash } }
...
async function compareCommand(args: Record<string, string>): Promise<JsonObject> { const before = await validateBundlePath(required(args, 'before')); const after = await validateBundlePath(required(args, 'after')); const beforeSnapshot = before.payload['comparison-snapshot'] as ComparisonSnapshot ?? comparisonSnapshot(before.payload); const afterSnapshot = after.payload['comparison-snapshot'] as ComparisonSnapshot ?? comparisonSnapshot(after.payload); const { compareSnapshots } = await import('@voce/core'); const report = compareSnapshots({ caseId: after.manifest.case.id, beforeRevision: before.manifest.case.revision, afterRevision: after.manifest.case.revision, before: beforeSnapshot, after: afterSnapshot }); const out = args.out ? path.resolve(args.out) : undefined; if (out) await atomicWrite(out, textBytes(report)); return out ? { status: 'ok', reportHash: report.reportHash, output: safeDisplayPath(out) } : report as unknown as JsonObject }
async function packInspectCommand(source: string): Promise<JsonObject> { const loaded = await loadPack(source); return { status: 'ok', packId: loaded.descriptor.manifest.packId, version: loaded.descriptor.manifest.version, kind: loaded.descriptor.manifest.kind, manifestHash: loaded.descriptor.manifestHash, packageDigest: loaded.descriptor.packageDigest, distributionDigest: loaded.descriptor.distributionDigest, fixtureSuites: loaded.definition.contributions.fixtureSuites.map((suite) => ({ id: suite.id, caseIds: suite.cases.map((item) => isObject(item) && typeof item.id === 'string' ? item.id : 'invalid') })) } }
...
async function packTestCommand(source: string): Promise<JsonObject> { const loaded = await loadPack(source); const registry = createScenarioPackRegistry(); registry.register(loaded.source); const resolution = registry.resolve({ root: { packId: loaded.descriptor.manifest.packId, versionRange: loaded.descriptor.manifest.version }, extensions: [] }); if (resolution.status !== 'resolved') throw new CliError('PACK_RESOLUTION_BLOCKED', 'ScenarioPack resolution is blocked.', EXIT.contract); const results: JsonObject[] = []; for (const fixture of loaded.fixtures) { const profile = profileFor(fixture.profileId ?? MOCK_NATIVE_TRANSPARENT_PROFILE.id); const input = fixtureM5ExecutionInput(profile); const result = executeOffline(input); const expected = fixture.expectedStatus ?? 'completed'; const accepted = result.status === expected || (expected === 'needs_review' && result.status === 'completed'); results.push({ id: fixture.id, status: accepted ? 'passed' : 'failed', expectedStatus: expected, observedStatus: result.status, executionCode: result.code, reasons: result.reasons }) } const failed = results.some((item) => item.status === 'failed'); return { status: failed ? 'failed' : 'passed', packId: loaded.descriptor.manifest.packId, resolutionHash: resolution.report.reportHash, fixtureCount: results.length, results } }
async function doctorCommand(): Promise<JsonObject> { const major = Number(process.versions.node.split('.')[0]); return { status: major >= 20 ? 'ok' : 'blocked', node: { version: process.versions.node, supported: major >= 20 }, contracts: { schemaVersion: BUNDLE_MANIFEST_SCHEMA_VERSION, packageVersion: CONTRACTS_VERSION }, paths: { explicitOnly: true }, providers: { default: 'disabled', mock: 'offline', networkProbe: false }, credentials: { inspected: false } } }
