...
async function semanticForRun(run: NonNullable<OfflineExecutionResult['run']>, artifacts: ArtifactHandle[], profile: typeof MOCK_IMAGE_PROFILE): Promise<{ report: import('@voce/contracts').SemanticReviewReport; receipt: import('@voce/contracts').StepReceipt; remote: import('@voce/contracts').RemoteCallRun }> {
...
async function runCommand(args: Record<string, string>): Promise<JsonObject> {
  ...
  const fixture = bundle.payload.fixture as SourceFixtureCase | undefined; let semanticProposal: import('@voce/contracts').SemanticReviewReport | undefined; const semanticReceipts: import('@voce/contracts').StepReceipt[] = []; const reconciliation: import('@voce/contracts').RemoteCallRun[] = []; if (fixture?.semanticReview) { const semantic = await semanticForRun(run, [structuralArtifact, ...artifacts], profileFor(String((bundle.payload.profile as Record<string, unknown>)?.id ?? MOCK_IMAGE_PROFILE.id))); semanticProposal = semantic.report; semanticReceipts.push(semantic.receipt); reconciliation.push(semantic.remote) }
