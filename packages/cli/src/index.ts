...
import {
  FIXTURE_M6_ALPHA_PNG, fixtureChangeIntent, fixtureM4ConstraintInput, fixtureM5Candidate, fixtureM5ExecutionInput, fixtureM5GuardInput, fixtureM5PromptIR, fixtureM6Artifact, fixtureM6Authorization, fixtureScopePlan,
} from '@voce/testkit'
...
async function loadProfile(filePath: string): Promise<typeof MOCK_NATIVE_TRANSPARENT_PROFILE> { const object = record(await readJson(path.resolve(filePath), 'PROFILE_FILE_UNSAFE'), 'PROFILE_JSON_INVALID'); return profileFor(stringField(object, 'id', 'PROFILE_INVALID')) }
...
interface CompileResult { loaded: LoadedPack; resolution: ResolvedPackResolution; caseSpec: CaseSpec; profile: typeof MOCK_NATIVE_TRANSPARENT_PROFILE; fixture: SourceFixtureCase; input: OfflineExecutionInput; prompt: PromptCandidateIR; acceptance: JsonObject }
...
async function validateBundlePath(bundlePath: string): Promise<{ manifest: BundleManifest; payload: Record<string, unknown> }> {
  ...
  for (const entry of files) { const filePath = path.join(directory, ...entry.path.split('/')); await regularFile(filePath, 'BUNDLE_ENTRY_UNSAFE'); const bytes = await readFile(filePath).catch(() => { throw new CliError('BUNDLE_FILE_MISSING', 'Bundle inventory file is missing.', EXIT.contract) }); if (bytes.byteLength !== entry.byteLength || bytesHash(bytes) !== entry.sha256) throw new CliError('BUNDLE_FILE_HASH_MISMATCH', 'Bundle file hash or length does not match its manifest.', EXIT.contract) }
...
async function packTestCommand(source: string): Promise<JsonObject> {
  ...
  for (const fixture of loaded.fixtures) {
    ...
    const artifacts = run.outputArtifacts ?? []; const structuralArtifact = fixtureM6Artifact(`structural-${run.id}`, FIXTURE_M6_ALPHA_PNG, 'image/png', 'transparent-image'); const outputContract: OutputContract = { artifactKind: 'image', dataType: 'image', mediaTypes: ['image/png'], cardinality: { min: 1, max: 1 }, background: 'transparent', allowAlpha: true }; const structural = validateStructuralImage({ schemaVersion: 'voce.structural-validation-input/v1alpha1', id: `structural-${run.id}`, artifacts: [{ artifact: structuralArtifact, bytes: FIXTURE_M6_ALPHA_PNG }], outputContract, expectedCardinality: { min: 1, max: 1 } }); let semanticProposal: import('@voce/contracts').SemanticReviewReport | undefined; const semanticReceipts: import('@voce/contracts').StepReceipt[] = []; const reconciliation: import('@voce/contracts').RemoteCallRun[] = []; if (fixture.semanticReview) { const semantic = await semanticForRun(run, [structuralArtifact, ...artifacts], profile); semanticProposal = semantic.report; semanticReceipts.push(semantic.receipt); reconciliation.push(semantic.remote) }
  ...
  const failed = results.some((item) => item.status === 'failed'); return { status: failed ? 'failed' : 'passed', packId: loaded.descriptor.manifest.packId, resolutionHash: resolution.report.reportHash, fixtureCount: results.length, results }
}
