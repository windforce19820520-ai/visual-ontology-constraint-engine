...
test('CLI help and version have stable machine output', () => {
  ...
  const version = invoke(['--version']); assert.equal(version.status, 0); assert.deepEqual(version.json, { status: 'ok', version: '0.1.0-rc.5' })
})
...
test('three vertical fixtures compile, run with Mock, and render a static trace', async () => {
  ...
  try {
    ...
    for (const [name, profile] of cases) {
      ...
      const compile = invoke(['case', 'compile', '--case', fixture(`cases/${name}.json`), '--scenario', fixture(`packs/${name}`), '--profile', fixture(`profiles/${profile}.json`), '--out', compiled]); assert.equal(compile.status, 0); const compileAcceptance = JSON.parse(await readFile(path.join(compiled, 'acceptance.json'), 'utf8')) as Record<string, unknown>; const compileAssertions = compileAcceptance.assertions as Array<Record<string, unknown>>; assert.ok(compileAssertions.length > 0); assert.ok(compileAssertions.every((item) => item.status === 'passed')); assert.ok(compileAssertions.every((item) => /^sha256:[0-9a-f]{64}$/.test(String(item.observedHash)))); const coreProbes = Object.values(compileAcceptance.coreProbes as Record<string, Record<string, unknown>>); for (const probe of coreProbes) { assert.equal(probe.presetReferenceCountBefore, probe.presetReferenceCountAfter); assert.notEqual(probe.presetConstraintHashBefore, probe.presetConstraintHashAfter); assert.match(String(probe.presetReferencePlanHashBefore), /^sha256:[0-9a-f]{64}$/); assert.match(String(probe.presetReferencePlanHashAfter), /^sha256:[0-9a-f]{64}$/) } const compiledCase = JSON.parse(await readFile(path.join(compiled, 'case.json'), 'utf8')) as { scenario: { root: { packId: string; versionRange: string } } }; const compiledScenario = JSON.parse(await readFile(path.join(compiled, 'scenario.json'), 'utf8')) as { lock: { entries: Array<{ packId: string; version: string }> } }; const compiledManifest = JSON.parse(await readFile(path.join(compiled, 'manifest.json'), 'utf8')) as { pins: { scenario: { version: string } } }; const rootLock = compiledScenario.lock.entries.find((entry) => entry.packId === compiledCase.scenario.root.packId); assert.ok(rootLock); assert.equal(compiledCase.scenario.root.versionRange, rootLock.version); assert.equal(compiledManifest.pins.scenario.version, rootLock.version)
