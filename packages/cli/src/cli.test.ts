...
test('compile, run, trace, and compare reject output paths that overlap inputs', async () => {
  const directory = await mkdtemp(path.join(tmpdir(), 'voce-output-overlap-')); const compiled = path.join(directory, 'compiled'); const run = path.join(directory, 'run'); const casePath = fixture('cases/product-shot.json'); const scenarioPath = fixture('packs/product-shot'); const profilePath = fixture('profiles/mock-native-transparent.json')
  try {
