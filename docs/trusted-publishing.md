...
`@voce-engine/playground` was a new npm name in RC.5, and npm requires a package to exist before its package-level trusted publisher can be configured. The owner therefore completed a one-time, explicitly authorized `0.0.0-bootstrap.0` namespace bootstrap under the `bootstrap` tag through an interactive 2FA-protected npm action. No token was stored in Git or workflow files. npm also assigned that first package record to `latest`; the owner elected not to retarget it. The reviewed package is published as `next -> 0.1.0-rc.5`, so RC.5 documentation always uses `@next` or the exact version and never recommends an unversioned Playground install. The package-level trusted publisher was restricted to this repository, `publish-npm.yml`, and the `npm publish` action before the RC.5 workflow was dispatched.
