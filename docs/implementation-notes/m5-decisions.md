...
- Mock adapters return hash-addressed virtual `ArtifactHandle` metadata only. They do not make network calls, download URLs, create image bytes, or invoke paid providers. Standard image and JPEG-normalization plans use the same provider-neutral adapter port; optional generic postprocessors remain host-registered extensions.
