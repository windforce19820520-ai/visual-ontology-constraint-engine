...
Future npm releases use the tokenless GitHub Actions OIDC flow described in [npm trusted publishing](trusted-publishing.md). npm provenance generated by that external publication is distinct from the local `build-manifest.json`; neither artifact substitutes for the other.
