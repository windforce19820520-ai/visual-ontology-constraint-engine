...
This repository produces an auditable local release candidate before any external publication. Publication is a separate, explicitly owner-authorized operation. `v0.1.0-rc.2` is published as an npm `next` candidate and a GitHub prerelease; it is not production-ready.
...
RC.2 and future npm releases use the tokenless GitHub Actions OIDC flow described in [npm trusted publishing](trusted-publishing.md). npm provenance generated by that external publication is distinct from the local `build-manifest.json`; neither artifact substitutes for the other.
