...
This Host is the fifth public package published in `v0.1.0-rc.5` under npm `next`. It passed local product acceptance, release gates, OIDC publication, and an exact-version public-registry clean consumer on 2026-08-21. The package still starts safely on loopback by default. The current source also includes the reviewed single-instance public-deployment controls and generic Nginx/systemd templates. An authorized temporary-host acceptance deployment was completed separately on 2026-08-21; that operational evidence does not turn RC.5 into a production-readiness, stable-domain, or multi-instance claim.
...
Rendering is disabled by default only for source checkouts, CI, and ordinary local development. That default is not the Public Playground product contract: a public user-facing deployment is expected to enable Cloudflare, Seedream, and Grok transports. For local Mock-only generation, set `PLAYGROUND_ENABLE_MOCK_RENDER=1` and `PLAYGROUND_DEVELOPMENT_MODE=1` before starting; Mock is hidden from the ordinary product selector but remains available explicitly to development/tests. Seedream is the recommended high-quality BYOK option and is enabled with `PLAYGROUND_ENABLE_SEEDREAM_TRANSPORT=1`. Grok is the optional high-quality BYOK option and is enabled with `PLAYGROUND_ENABLE_GROK_TRANSPORT=1`; its allow-listed JSON HTTP adapter is tested with Mock HTTP only. Cloudflare is a free experimental preview enabled only when `PLAYGROUND_ENABLE_CLOUDFLARE_TRANSPORT=1` and both server-side credential variables are present. BYOK values are cleared immediately, never stored, and never retried automatically.

Virtual Try-On requires one person image plus clothing. The Browser presents two exclusive paths: one Full outfit image, or Top / Bottom where either piece or both may be uploaded. It does not ask users to classify garment categories; the selected slot bounds the replacement region, while API callers may still provide optional allow-listed category/structure metadata. Uploading only an upper garment preserves the original lower garment; uploading only a lower garment preserves the original upper garment; uploading both or one full-outfit reference replaces both regions. Footwear, fit, pose, and up to four typed accessory references are optional. The `Optional refinements — Fit, Footwear, Pose, Accessories` section is open by default so the accessory uploader is visible without another click. Each accessory keeps the meaningful ScenarioPack-declared type, placement, side, and appearance combination; when none is supplied, the original accessories are preserved. Try-On does not expose composition presets and preserves the source pose and camera framing unless a pose reference is supplied.
...
- `POST /api/upload` (bounded request-scoped memory; PNG/JPEG/WebP privacy metadata is removed; bytes are never persisted)
...
- `GET /api/generated/:id` (same-session HttpOnly Cookie authorization; no session query parameter)
- `DELETE /api/session` (clears request-scoped uploads and results and expires the Cookie)
...
The ordinary selector order is Seedream recommended BYOK, Grok optional BYOK, and Cloudflare free experimental preview. Mock is a development/test profile only. Cloudflare uses an operator-managed server credential; the browser never shows a Cloudflare key field. The public Host can therefore provide free calls within its shared preview quota, but this path is only an example of the complete model-call workflow and is not VOCE's quality representative. Person identity consistency, virtual try-on quality, Cosplay character reproduction, and composition control can be unsatisfactory. The profile accepts at most four ordered references, maps them mechanically to multipart `input_image_0` through `input_image_3`, and blocks any input whose width or height is 512 or greater before transport. A successful response is decoded and held in request-scoped Host memory for 15 minutes so the browser can display the generated image; neither input nor output bytes are written to disk. The shared free allocation is capped at 10,000 Neurons per UTC day and resets at 00:00 UTC; exhaustion, Cloudflare account-limited responses, and transport failures fail closed with no retry, paid continuation, or Provider switch. The full configuration-name example is [`cloudflare.env.example`](cloudflare.env.example); no secret values belong in the repository.
...
Public availability is transport-based, not key-presence-based. Seedream and Grok remain available choices when their reviewed transports are deployed even though the server stores no user key; a missing BYOK value rejects only that user's Generate attempt with an understandable prompt. Cloudflare availability depends on the operator-managed credential and transport. A Provider is marked unavailable only when its reviewed transport is not deployed or the compiled plan exceeds its real capability. Quotas, consent, request-scoped secrets, and fail-closed error handling continue to apply to every Generate attempt.

Seedream supports the current four/five-reference flows and up to ten declared Cosplay references under the profile. Its BYOK transport is fixed to the allow-listed Ark endpoint and the previously real-provider-tested `doubao-seedream-5-0-pro-260628`, sends one request with no retry, downloads the single returned image into request-scoped memory, and exposes it through the same 15-minute no-store result route. Grok Imagine accepts at most three references, so a four-reference Try-On plan compiles for inspection but cannot be generated with Grok; the Host does not drop a reference or switch Provider silently. Its adapter sends the official `application/json` edit shape with request-scoped Base64 data URIs and downloads only allow-listed xAI HTTPS output; deterministic tests inject Mock HTTP and make no xAI call. Provider calls preserve the Guard-accepted positive sections, prohibitions, typed parameters, output contract, and reference order.

Public-mode startup, reverse-proxy requirements, Cookie/trusted-proxy rules, in-memory quota durability, and single-host prerequisites are documented in [`docs/public-playground-deployment-readiness.md`](../docs/public-playground-deployment-readiness.md). The generic single-host Nginx, systemd, safe source-default environment, and explicitly activated Provider egress templates are in [`deploy/public-playground`](../deploy/public-playground/README.md). The data lifecycle is documented in [`docs/public-playground-privacy.md`](../docs/public-playground-privacy.md).

The separately authorized temporary-host deployment and Mock-only online acceptance evidence are recorded in [`docs/acceptance/public-playground-single-instance.md`](../docs/acceptance/public-playground-single-instance.md).
