...
This Host is the fifth public package published in `v0.1.0-rc.5` under npm `next`. It passed local product acceptance, release gates, OIDC publication, and an exact-version public-registry clean consumer on 2026-08-21. The package still starts safely on loopback by default. The current source adds public-deployment readiness controls, but it is not a public deployment or production-readiness claim; a separately authorized PR B remains responsible for infrastructure, secrets, domain/TLS, monitoring, and online acceptance.
...
Public-mode startup, reverse-proxy requirements, Cookie/trusted-proxy rules, in-memory quota durability, and PR B prerequisites are documented in [`docs/public-playground-deployment-readiness.md`](../docs/public-playground-deployment-readiness.md). The generic single-host Nginx, systemd, safe source-default environment, and explicitly activated Provider egress templates are in [`deploy/public-playground`](../deploy/public-playground/README.md). The data lifecycle is documented in [`docs/public-playground-privacy.md`](../docs/public-playground-privacy.md).
